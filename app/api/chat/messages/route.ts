...
// Single global conversation chat API.
import { createAdminClient, createClient } from '@/lib/supabase/server';
import { NextResponse } from 'next/server';
import type { ChatAttachment, ChatMessage } from '@/types';
import { sendPushToRoomParticipants } from '@/lib/backend/push';

const GLOBAL_LIBRARY_ID = 'global-library';
const GLOBAL_ROOM_ID = 'global-chat';
const CHAT_BUCKET = 'chat-attachments';

async function verifyMembership(supabase: ReturnType<typeof createClient>, userId: string) {
  const { data } = await supabase
    .from('library_members')
    .select('role')
    .eq('library_id', GLOBAL_LIBRARY_ID)
    .eq('user_id', userId)
    .maybeSingle();
  return data;
}
...
export async function GET(req: Request) {
  ...
  const membership = await verifyMembership(supabase, user.id);
  ...
  const clearTime = await getClearTime(db, GLOBAL_ROOM_ID, user.id);
...
export async function POST(req: Request) {
  ...
  if (error) {
    if (error.code === '42P01') return NextResponse.json({ message: { id: body.id ?? crypto.randomUUID(), roomId: GLOBAL_ROOM_ID, userId: user.id, userName: senderName, avatarColor, avatarUrl, content, ts: Date.now() } });
  ...
  if (attachment?.storagePath) {
    ...
    if (attachError) console.warn('[api/chat/messages] attachment metadata failed:', attachError);
  }
  ...
  sendPushToRoomParticipants(GLOBAL_ROOM_ID, user.id, pushPayload, false);
...
export async function PATCH(req: Request) {
  ...
  if (action === 'clear') {
    const { error } = await db.from('message_clears').upsert({ room_id: GLOBAL_ROOM_ID, user_id: user.id, cleared_at: new Date().toISOString() }, { onConflict: 'room_id,user_id' });
  ...
  if (action === 'read') {
    ...
    const rows = messageIds.map((messageId: string) => ({ room_id: GLOBAL_ROOM_ID, message_id: messageId, user_id: user.id, delivered_at: now, read_at: now }));
    if (rows.length) {
  ...
  if (action === 'delivered') {
    const messageIds = Array.isArray(body.messageIds) ? body.messageIds.slice(0, 200) : [];
    const now = new Date().toISOString();
    const rows = messageIds.map((messageId: string) => ({ room_id: GLOBAL_ROOM_ID, message_id: messageId, user_id: user.id, delivered_at: now }));
    if (rows.length) {
      const { error } = await db.from('message_read_receipts').upsert(rows, { onConflict: 'message_id,user_id' });
  ...
  const { data: msg } = await db.from('messages').select('sender_id').eq('id', messageId).eq('room_id', GLOBAL_ROOM_ID).maybeSingle();
